...
export class GupshupV3Service implements WhatsappInterfaceService {
    ...
    async handleIncomingMessage(payload: MetaWhatsappWebhookEvent, channelConfigToken: string, workspaceId?: string) {
        await this.gupshupV3IncomingService.handleIncomingMessage(payload, channelConfigToken, workspaceId);
    }

    async handleIncomingAck(payload: MetaWhatsappWebhookEvent, channelConfigToken: string, workspaceId?: string) {
        await this.gupshupV3IncomingService.handleIncomingAck(payload, channelConfigToken, workspaceId);
    }
    ...
    async handleIncomingTemplateEvent(payload: any, channelConfigToken: string) {
        ...
        if (event === 'template_category_update') {
            return await this.gupshupV3IncomingService.handleIncomingTemplateCategoryChanged(
        ...
        if (event === 'message_template_status_update') {
            return await this.gupshupV3IncomingService.handleIncomingTemplateStatusChanged(
                payload as MetaWhatsappIncomingTemplateEvent,
                channelConfigToken,
            );
        }

        console.log('GupshupV3Service Template event not found', payload);
        return;
    }
    ...
    async createTemplateMetaWhatsapp(
        ...
        try {
            ...
            Sentry.captureEvent({
                message: `${GupshupV3Service.name}.createTemplateMetaWhatsapp`,
                extra: {
                    error: err,
                },
            });

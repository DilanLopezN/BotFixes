...
export class CacheService {
    ...
    public async setObject(entity: Document | any, key?: string | ObjectId, redisExpiration?: number): Promise<void> {
        ...
        try {
            ...
            const _key = this.getKey(key, entity);
            if (_key && entity) {
                await client.set(_key, entity, 'EX', redisExpiration || process.env.REDIS_CACHE_EXPIRATION || 120);
            }
        } catch (e) {
            console.log('cache set', e);
        }
    ...
    public async removeKeysByPattern(pattern: string, patternsToIgnore?: string[]): Promise<void> {
        const prefix = 'API:';

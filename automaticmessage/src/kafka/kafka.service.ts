...
import {
  KAFKA_ADMIN_INJECT_TOKEN,
  KAFKA_INJECT_TOKEN,
  KAFKA_PRODUCER_INJECT_TOKEN,
} from './kafka.module';
import * as moment from 'dayjs';
import { getTopicPrefix } from 'kissbot-core';
import { shouldStartKafka } from '../miscellaneous/utils';
...
export class KafkaService {
  ...
  async getKafkaConsumer(data: StartConsumerData): Promise<Consumer> {
    ...
    if(!shouldStartKafka()) {
      this.logger.warn(
        'Omitting start kafka consumer TOPIC: ' +
          data.topic +
          ' - GROUP:' +
    ...
    consumer.on('consumer.connect', (ev) => {
      ...
      this.consumerGroupResume[consumerGroupId] = {
        ...
        disconnectedTimestamp: 0,
        disconnectCount:
          this.consumerGroupResume[consumerGroupId].disconnectCount,
      };
    });
    consumer.on('consumer.disconnect', (ev) => {
      this.consumerGroupResume[consumerGroupId] = {
        ...
        disconnectCount:
          this.consumerGroupResume[consumerGroupId].disconnectCount + 1,
      };
      this.logger.log(
        `Disconnected consumer on topic ${topicName}: ${JSON.stringify(ev)}`,
      );
    });
  ...
  async ping(): Promise<boolean> {
    ...
    const disconnectedList = Object.values(this.consumerGroupResume).filter(
      (resume) => {
        const diff = now - resume.disconnectedTimestamp;
        return (
          resume.status == ConsumerGroupStatus.disconnected &&
          diff > this.healthCheckTimeThreshold
        );
      },
    );

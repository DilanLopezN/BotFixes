...
export class FeegowConfirmationService {
  ...
  private async listSchedulesToConfirmData(
    ...
    // Filter doctors: omit specific doctors
    if (erpParams?.omitDoctors?.length && feegowSchedules.length) {
      feegowSchedules = feegowSchedules.filter(
        (schedule) => !erpParams.omitDoctors.includes(String(schedule.profissional_id)),
      );
    }

    // Filter doctors: include only specific doctors
    if (erpParams?.includeDoctors?.length && feegowSchedules.length) {
      feegowSchedules = feegowSchedules.filter((schedule) =>
        erpParams.includeDoctors.includes(String(schedule.profissional_id)),
      );
    }

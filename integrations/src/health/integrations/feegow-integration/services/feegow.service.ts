...
export class FeegowService implements IIntegratorService {
  ...
  public async createPatient(integration: IntegrationDocument, { patient }: CreatePatient): Promise<Patient> {
    try {
      ...
      // A Feegow retornou o id do paciente em dois formatos: dentro de um objeto no content ou diretamente no content
      const pacienteId =
        typeof response.content === 'object' && response.content !== null && 'paciente_id' in response.content
          ? response.content.paciente_id
          : response.content;

      if (!pacienteId) {
        throw HTTP_ERROR_THROWER(HttpStatus.BAD_REQUEST, 'Unable to create patient', HttpErrorOrigin.INTEGRATION_ERROR);
      }

      const createdPatient = await this.getPatientByCode(integration, String(pacienteId));

      if (!createdPatient) {
        throw HTTP_ERROR_THROWER(
          HttpStatus.BAD_REQUEST,
          'Unable to obtain patient on creation',

...
export class PhillipsEntitiesService {
  ...
  private getResourceFilters(targetEntity: EntityType, filters: CorrelationFilter): RequestParams {
    ...
    if (targetEntity === EntityType.insurance && filters.hasOwnProperty(EntityType.organizationUnit)) {
    ...
    if (targetEntity === EntityType.speciality && filters.hasOwnProperty(EntityType.doctor)) {
    ...
    if (targetEntity === EntityType.procedure) {
      ...
      // adicionar outros filtros específicos de procedure conforme necessidade
    }
  ...
  // ========== INSURANCE PLANS ==========
  // Extraídos do retorno de getInsurances (insurancePlanEntity)

  private async listInsurancePlans(
    integration: IntegrationDocument,
    requestFilters: RequestParams,
  ): Promise<IInsurancePlanEntity[]> {
    throw INTERNAL_ERROR_THROWER('PhillipsEntitiesService.listInsurancePlans', 'Not implemented');
  }

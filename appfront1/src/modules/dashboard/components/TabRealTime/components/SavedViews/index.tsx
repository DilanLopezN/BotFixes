...
const SavedViews: FC<SavedViewsProps & I18nProps> = ({
    ...
    const USER_SETTING_TYPE = 'realtime-reports';
    ...
    const saveCurrentView = async () => {
        ...
        // Gera a configuração atual no momento de salvar
        const configToSave = generateConfig ? generateConfig() : currentViewConfig;

        if (!configToSave && !editingView) {
        ...
        try {
            ...
            if (editingView) {
                ...
                const updatedView = await DashboardService.updateUserSetting(
                    ...
                    editingView.key,
                    {
                        value: configToSave || editingView.value,
                        label: viewName.trim(),
                    }
                );
    ...
    const handleUpdateViewFilters = async (view: SavedView, e: React.MouseEvent) => {
        ...
        // Gera a configuração atual no momento de atualizar
        const configToSave = generateConfig ? generateConfig() : currentViewConfig;
